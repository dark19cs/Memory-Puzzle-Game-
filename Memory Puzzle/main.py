...
def pause_game():
    global paused
    global pre_pause_muted
    paused = not paused
    status.config(text="⏸ Paused" if paused else "▶ Resumed")
    # pause or resume timer
    if paused:
        # remember previous mute state and mute audio during pause
        try:
            pre_pause_muted = sc.muted
        except Exception:
            pre_pause_muted = None
        sc.muted = True
        timer.pause()
        ...
        # restore mute state
        if pre_pause_muted is not None:
            sc.muted = pre_pause_muted
        else:
            sc.muted = False
